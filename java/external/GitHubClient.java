...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword,"TUF-8");
		}catch(UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		String url = String.format(URL_TEMPLATE, keyword, lat, lon);//prepare url
		CloseableHttpClient httpClient = HttpClients.createDefault();//send request
		try {
			CloseableHttpResponse response= httpClient.execute(new HttpGet(url));
			if(response.getStatusLine().getStatusCode()!=200) {
				return new ArrayList<>();//check if the status is right, 200 is http status code for okay
			}
			HttpEntity entity = response.getEntity();//get the body of the http request
			if(entity==null) {
	...
	private List<Item> getItemList(JSONArray array) {
		...
		// We need to get keywords from multiple text in one request since MonkeyLearnAPI has a limitation on request per minute.
